...
old = """def channel_summary_html(channels,total):
...
"""
new = """def channel_summary_html(channels,total):
    items=(("VENDEDORES FRANQUIA",channels.get("VENDEDORES FRANQUIA",0),"blue"),("CANAL NACIONAL",channels.get("CANAL NACIONAL",0),"green"))
    cards=[]
...
"""
if old not in s:
    raise SystemExit("channel_summary_html não encontrado")
s = s.replace(old, new, 1)

old = """def team_performance_card_html(title,goal,metrics,tone="internal"):
...
"""
new = """def team_performance_card_html(title,goal,metrics,tone="internal"):
    width=min(100,max(0,metrics["attainment"]*100))
...
"""
if old not in s:
    raise SystemExit("team_performance_card_html não encontrado")
s = s.replace(old, new, 1)

old = """def production_channel_dashboard_html(channels,total,summary,cfg,data_until,updated):
...
"""
new = """def production_channel_dashboard_html(channels,total,summary,cfg,data_until,updated):
    internal_goal=int(cfg.get("meta_equipe_interna",0) or 0); external_goal=int(cfg.get("meta_equipe_externa",0) or 0)
    internal=team_performance_metrics(summary,"Equipe Interna",internal_goal); external=team_performance_metrics(summary,"Equipe Externa",external_goal)
