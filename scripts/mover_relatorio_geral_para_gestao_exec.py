...
'''
if old not in text:
    raise SystemExit('Bloco de Gestão não encontrado ou já alterado')
text=text.replace(old,new,1)

old_call='''        render_general_report(st,team,rows,cfg,summary,all_days,elapsed,official,color)\n'''
if old_call not in text:
    raise SystemExit('Chamada da Visão Geral não encontrada ou já removida')
text=text.replace(old_call,'',1)

path.write_text(text,encoding='utf-8')
print('OK: relatório completo na Gestão e removido da Visão Geral.')
