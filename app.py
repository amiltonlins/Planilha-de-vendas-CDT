...
import copy, csv, html, io, json, re, tempfile, unicodedata, zipfile
...
def prepare_config(base,rows,month,year):
 ...
 for name in sorted({x["vendedor"] for x in rows}):
  ...
  sellers.append({"vendedor":name,"setor":old.get("setor",sample["setor"]),"categoria":old.get("categoria",inferred),"ativo":old.get("ativo",True),"experiencia":old.get("experiencia",False),"meta_individual":old.get("meta_individual",70),"trabalha_sabado":old.get("trabalha_sabado",True),"trabalha_domingo":old.get("trabalha_domingo",False),"data_inicio":old.get("data_inicio",f"{year}-01-01"),"data_desligamento":old.get("data_desligamento",""),"folgas":old.get("folgas",[])})
...
def regular(summary):return [x for x in summary if normalize_text(x["categoria"]) not in {"website","adm","freelance"}]
...
def render_app():
 ...
 with st.sidebar:
  st.markdown("### PAINEL COMERCIAL"); area=st.radio("Navegação",["VISÃO GERAL","VENDEDORES","SEMANAL","COMISSÕES","CONFIGURAÇÕES"],label_visibility="collapsed")
  month=st.selectbox("Mês",range(1,13),index=base["mes"]-1); year=int(st.number_input("Ano",2020,2100,value=base["ano"])); uploaded=st.file_uploader("RELATÓRIO XLSX OU CSV",type=["xlsx","csv"]); demo=st.checkbox("Usar dados de demonstração",value=not uploaded)
 if not uploaded and not demo:st.info("Importe o relatório de vendas para iniciar.");return
 try:
  raw=rows_from_csv((ROOT/"dados_exemplo.csv").read_bytes()) if not uploaded else (rows_from_xlsx(uploaded.getvalue()) if uploaded.name.lower().endswith(".xlsx") else rows_from_csv(uploaded.getvalue()))
  rows,mapping=canonicalize(raw,base); cfg=prepare_config(base,rows,month,year)
  if "cfg" in st.session_state and st.session_state.get("competencia")==f"{month}/{year}":cfg=st.session_state.cfg
 ...
 if area=="VISÃO GERAL":
  ...
  cards(st,[(f"VENDEDORES {k.upper()}",v,k.lower() if k!="Azul" else "cyan","") for k,v in counts.items()])
  ...
  st.markdown('<div class="section">CANAIS SEPARADOS</div>',unsafe_allow_html=True); cards(st,[(x["categoria"].upper(),x["vendas"],"cyan",f'Projeção {x["projecao"]}') for x in extra] or [("WEBSITE",0,"cyan",""),("ADM",0,"cyan",""),("FREELANCE",0,"cyan","")],3)
  ...
  st.markdown('<div class="section">CONFIGURAÇÕES MENSAIS</div>',unsafe_allow_html=True)
  with st.form("config"):
   a,b,c=st.columns(3); cfg["meta_empresa"]=int(a.number_input("Meta mensal",1,value=int(cfg["meta_empresa"]))); cfg["limite_cenario_maior"]=int(b.number_input("Limite do cenário maior",1,value=int(cfg["limite_cenario_maior"]))); cfg["dia_referencia"]=int(c.number_input("Dia de referência",1,31,value=min(int(cfg["dia_referencia"]),31)))
   ...
   for i,item in enumerate(cfg["premiacao_semanal"]):item["vendas"]=int(cols[i].number_input(f"Faixa {i+1}",1,value=int(item["vendas"]),key=f"fq{i}")); item["premio"]=cols[i].number_input("Prêmio R$",0.,value=float(item["premio"]),key=f"fp{i}")
   st.markdown("#### CADASTRO DE VENDEDORES")
   for i,s in enumerate(cfg["vendedores"]):
    with st.expander(s["vendedor"]):
     a,b,c,d=st.columns(4); s["setor"]=a.text_input("Setor",s["setor"],key=f"set{i}"); s["ativo"]=b.checkbox("Ativo",s.get("ativo",True),key=f"at{i}"); s["experiencia"]=c.checkbox("Em experiência",s["experiencia"],key=f"ex{i}"); s["meta_individual"]=int(d.number_input("Meta individual",1,value=int(s["meta_individual"]),key=f"me{i}")); a,b,c,d=st.columns(4); s["trabalha_sabado"]=a.checkbox("Trabalha sábado",s.get("trabalha_sabado",True),key=f"sa{i}"); s["trabalha_domingo"]=b.checkbox("Trabalha domingo",s.get("trabalha_domingo",False),key=f"do{i}"); s["data_inicio"]=c.text_input("Data de início",s.get("data_inicio",""),key=f"di{i}"); s["data_desligamento"]=d.text_input("Desligamento",s.get("data_desligamento",""),key=f"dd{i}"); s["folgas"]=[v.strip() for v in st.text_input("Folgas/afastamentos (AAAA-MM-DD, separados por vírgula)",",".join(s.get("folgas",[])),key=f"fo{i}").split(",") if v.strip()]
   submitted=st.form_submit_button("APLICAR CONFIGURAÇÕES")
  if submitted:st.session_state.cfg=cfg; st.session_state.competencia=f"{month}/{year}"; st.success("Configurações aplicadas nesta sessão.")
  if st.button("SALVAR NO CONFIG.JSON"):
   (ROOT/"config.json").write_text(json.dumps(cfg,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); st.success("Configurações salvas.")
 if area!="CONFIGURAÇÕES":
