...
CSS="""<style>
#MainMenu,footer,header,[data-testid="stDecoration"]{display:none!important}.stApp{background:#F5F7FA;color:#172033}.block-container{padding:1.1rem 2rem 3rem;max-width:1800px}
.brand{background:#172033;color:#fff;padding:16px 22px;border-left:7px solid #06B6D4;margin-bottom:18px}.brand h1{font-size:1.55rem;margin:0}.brand p{margin:3px 0 0;color:#CBD5E1;font-size:.82rem}
.section{font-weight:900;color:#172033;border-bottom:2px solid #172033;padding:8px 0 5px;margin:20px 0 12px;letter-spacing:.04em}.metric{background:white;border:1px solid #D9E1E8;border-top:4px solid #0891B2;padding:13px 15px;min-height:92px;margin:5px 0;box-shadow:0 2px 5px #0f172a0d}
.metric span{display:block;font-size:.7rem;font-weight:800;color:#64748B}.metric strong{display:block;font-size:1.75rem;color:#172033}.metric small{font-size:.7rem;color:#64748B}.metric.green{border-top-color:#16A34A}.metric.yellow{border-top-color:#F59E0B}.metric.red{border-top-color:#DC2626}
.table-wrap{overflow:auto;max-height:610px;border:1px solid #CBD5E1;background:white}.report{border-collapse:collapse;white-space:nowrap;width:100%;font-size:.76rem}.report th{position:sticky;top:0;background:#172033;color:white;padding:8px 9px;z-index:2}.report td{border:1px solid #E2E8F0;padding:6px 8px;text-align:center}
.podium{background:white;border:1px solid #D9E1E8;padding:12px;margin:4px 0;border-left:6px solid}.stButton button,.stDownloadButton button{border-radius:2px;background:#172033;color:#fff;border:0;font-weight:800}.stSidebar{background:#172033}.stSidebar *{color:#E2E8F0}.stSidebar [aria-checked="true"]{background:#0891B2}
...
def render_app():
    ...
    base=json.loads((ROOT/"config.json").read_text(encoding="utf-8")); st.markdown('<div class="brand"><h1>PAINEL COMERCIAL — AFOGADOS</h1><p>Produção, performance, histórico e remuneração variável</p></div>',unsafe_allow_html=True)
    ...
    st.caption(f"Última atualização: {updated:%d/%m/%Y às %H:%M} · Dados acumulados até: {data_until:%d/%m/%Y}")
    ...
    if area=="VISÃO GERAL":
        ...
        tones={"Azul":"cyan","Verde":"green","Amarelo":"yellow","Vermelho":"red"}; cards(st,[(f"VENDEDORES {k.upper()}",v,tones[k],"") for k,v in counts.items()])
        st.markdown('<div class="section">RANKING DA EQUIPE</div>',unsafe_allow_html=True); ranking=sorted(team,key=lambda x:x["vendas"],reverse=True); podium=st.columns(3)
        for i,x in enumerate(ranking[:3]):podium[i].markdown(f'<div class="podium" style="border-left-color:{color(x)}"><b>{i+1}º {html.escape(x["vendedor"])}</b><br>{x["vendas"]} vendas · projeção {x["projecao"]}</div>',unsafe_allow_html=True)
        ...
        st.markdown(f'<div style="background:{c};color:white;padding:15px;font-weight:900">{html.escape(x["vendedor"])} · {html.escape(x["setor"])} · {status}</div>',unsafe_allow_html=True)
