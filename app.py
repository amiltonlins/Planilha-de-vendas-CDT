...
def _prepare_config_preserving_sellers(base, rows, month, year):
    ...
    existing = {_core.normalize_text(seller.get("vendedor", "")): seller for seller in sellers if _core.normalize_text(seller.get("vendedor", ""))}
    grouped = {}
    for row in rows:
    ...
    for key in sorted(grouped):
        ...
        inferred_category = next((label for label in ("Website", "ADM", "Freelance") if _core.normalize_text(label) in key), "Vendedor")
        inferred_team = _core.normalized_team(None, {"setor": sample.get("setor", "NÃO INFORMADO"), "categoria": inferred_category})
        new_seller = {
            "vendedor": name, "setor": sample.get("setor", "NÃO INFORMADO"), "equipe": inferred_team,
            "categoria": inferred_category, "pertence_franquia": False, "classificado": False,
            "ativo": False, "experiencia": False, "meta_individual": int(cfg.get("meta_padrao_vendedor", 70) or 70),
            "trabalha_sabado": True, "trabalha_domingo": False, "data_inicio": f"{year}-01-01",
            "data_desligamento": "", "folgas": [],
        }
...
def _clickable_team_card(title, goal, metrics, tone="internal", performance_counts=None):
    ...
    return (f'<a href="?team={quote(str(title))}" target="_self" style="display:contents;color:inherit;text-decoration:none" '
...
def _active_team_sellers(cfg, team_name):
    ...
    return [seller for seller in cfg.get("vendedores", []) if seller.get("ativo", False) and seller.get("equipe") == team_name
            and seller.get("pertence_franquia", True) and _core.normalize_text(seller.get("categoria", "")) not in blocked]
...
def _open_team_dialog_if_requested():
    ...
    try:
        ...
        summary, _, _, _ = _core.summarize(rows, cfg); _core.apply_team_labels(summary, cfg); team = _core.regular(summary)
    ...
    active_count = len(_active_team_sellers(cfg, team_name)); headcount_ideal = math.ceil(goal / 40) if goal > 0 else 0
    hires_needed = max(0, headcount_ideal - active_count); distribution = _core.team_performance_distribution(team, team_name)
...
def _install_login_viewport_fix():
    ...
    def render_login_without_scroll(st,cfg):
        ...
        st.markdown("""<style>
html,body{height:100%!important;overflow:hidden!important}[data-testid="stHeader"]{display:none!important}[data-testid="stAppViewContainer"],[data-testid="stMain"],.stMain{height:100dvh!important;min-height:100dvh!important;max-height:100dvh!important;overflow:hidden!important}[data-testid="stMainBlockContainer"],.main .block-container,.block-container{box-sizing:border-box!important;width:100%!important;max-width:100%!important;height:100dvh!important;min-height:100dvh!important;max-height:100dvh!important;margin:0!important;padding:0!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important}.results-login-wrap{box-sizing:border-box!important;width:100%!important;min-height:0!important;height:auto!important;margin:0 0 12px!important;padding:0 14px!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important}.results-login-logo{margin:0 auto 14px!important;max-width:min(220px,52vw)!important;max-height:82px!important}.results-login-spacer{height:6px!important}.st-key-results_access_form{box-sizing:border-box!important;width:min(360px,92vw)!important;margin:0 auto!important;padding:0!important}.results-access-error{margin:7px auto 0!important}[data-testid="stMainBlockContainer"] iframe{display:block!important;max-height:0!important;margin:0!important;padding:0!important}@media(max-width:700px){[data-testid="stMainBlockContainer"],.main .block-container,.block-container{height:100dvh!important;min-height:100dvh!important;max-height:100dvh!important;padding:0!important;justify-content:center!important}.results-login-wrap{min-height:0!important;height:auto!important;margin:0 0 10px!important;padding:0 12px!important;justify-content:center!important}.results-login-logo{width:min(185px,54vw)!important;max-height:70px!important;margin-bottom:12px!important}.results-login-title{font-size:1.08rem!important}.results-login-subtitle{margin-top:4px!important}.st-key-results_access_form{width:min(340px,90vw)!important}}
...
def _install_commercial_refresh_reference():
    ...
    try:
        nonlocals=inspect.getclosurevars(current_columns).nonlocals; original_columns=nonlocals.get("original_columns",current_columns)
    except Exception: original_columns=current_columns
...
.st-key-commercial_refresh button[kind],
.st-key-commercial_refresh [data-testid="stBaseButton-secondary"]{
...
.st-key-commercial_refresh button p{font-size:.82rem!important;font-weight:400!important;color:#64748B!important;line-height:1.25!important;white-space:nowrap!important}
.st-key-commercial_refresh button:hover,.st-key-commercial_refresh button:focus,.st-key-commercial_refresh button:active{background:transparent!important;border:0!important;box-shadow:none!important;color:#475569!important}
.st-key-commercial_refresh button:hover p,.st-key-commercial_refresh button:focus p,.st-key-commercial_refresh button:active p{color:#475569!important}
.st-key-commercial_refresh button:focus-visible{outline:1px dotted #94A3B8!important;outline-offset:2px!important}
</style>""",unsafe_allow_html=True)
